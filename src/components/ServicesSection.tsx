        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, index) => (
            <Link to={`/services/${service.slug}`} key={index} className="no-underline">
              <Card className="service-card-gradient border-[#533483] hover:border-[#A78BFA] transition-all h-full shadow-md">
                ...
                <CardContent>
                  <CardDescription className="text-[#A78BFA]">{service.description}</CardDescription>
                </CardContent>
              </Card>
            </Link>
          ))}
